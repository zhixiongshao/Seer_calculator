...
async function fetchAllResourceDetails(resource, count) {
  ...
  for (let offset = 0; offset < count; offset += SEERAPI_PAGE_SIZE) {
    ...
    const page = await fetchJson(`${SEERAPI_REST_BASE}/${resource}?offset=${offset}&limit=${limit}`);
    ids.push(...(page.results || []).map((item) => item.id));
  }

  const details = [];
  for (let i = 0; i < ids.length; i += 1) {
    details.push(await fetchJson(`${SEERAPI_REST_BASE}/${resource}/${ids[i]}`));
    if ((i + 1) % 20 === 0 || i + 1 === ids.length) {
      process.stdout.write(`\r${resource}: ${i + 1}/${ids.length}`);
    }
    if (i + 1 < ids.length) {
